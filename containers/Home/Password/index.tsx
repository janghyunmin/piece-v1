...
const Password = ({ navigation, route }: any) => {
  ...
  const mutation = useMutation(
    () => createPurchase(item.portfolioId, {
      purchasePieceVolume: status,
      storeVersion: Constants.manifest?.version!,
    }),
    {
      onSuccess: (res) => {
        // V1
        ...
        navigation.reset({ routes: [{
          name: 'result',
          params: { item, purchasePieceVolume: status, sessionId: res.sessionId },
        }]});
      },
      onError: (err: any) => {
        navigation.navigate('failedResult', {
          item,
        ...
        // navigation.reset({ routes: [{
        //   name: 'result',
        //   params: { item, purchasePieceVolume: status, sessionId: 123 },
        // }]});
      }
    }
  );
